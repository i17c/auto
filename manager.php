...
if($className!="" && $classDAOName!="" && $columnList!=""){
    ...
    if($primaryKey==""){
        $primaryKeyAry=ManagerGenerator::votePrimaryKey($columnAry);
    }else{
        $primaryKeyAry['id']=$primaryKey;
        $primaryKeyAry['type']=$type;
    }
    $content = ManagerGenerator::generate($className,$classDAOName,$primaryKeyAry);
    FileDownloader::download(ManagerGenerator::getManagerClassName($classDAOName).".java",$content);
