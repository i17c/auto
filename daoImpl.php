...
include_once("DaoImplGenerator.php");
...
if($className!="" && $classDAOName!="" && $columnList!=""){
    ...
    if($primaryKey==""){
        $primaryKeyAry=DaoImplGenerator::votePrimaryKey($columnAry);
    }else{
        $primaryKeyAry['id']=$primaryKey;
        $primaryKeyAry['type']=$type;
    }
    $content = DaoImplGenerator::generateImpl($className,$classDAOName,$classDAOImplName,$primaryKeyAry);
    FileDownloader::download($classDAOImplName.".java",$content);
